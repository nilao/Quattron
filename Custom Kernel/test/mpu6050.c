...
int lastRead = 0;
uint8_t processed_packet[8];
uint8_t received_packet[50];
uint8_t temp = 0;
uint8_t fifoCountL = 0;
uint8_t fifoCountL2 = 0;
uint8_t packetCount = 0x00;
uint8_t longPacket = false;
uint8_t firstPacket = true;
...
static uint8_t dmp_updates[29][9] =
{
	{
...
void regWrite(uint8_t reg ,uint8_t data){
	LDByteWriteI2C(fd_sensor,reg,data);
}

uint8_t regRead(uint8_t reg){
	write(fd_sensor,&reg,1);
	uint8_t buf;
	read(fd_sensor,&buf,1);
	return buf;
}
...
void MPU6050_init(){
	...
	regWrite(0x6D, 0x70);
	regWrite(0x6E, 0x06);
	uint8_t temp = regRead(0x6F);
	printf("Bank 1, Reg 6 = %x",temp);
...
void dmp_init(){
	...
	for(int i = 0; i < 7; i++){
		...
		for(uint8_t j = 0; j < 16; j++){
			...
			regWrite (MEM_START_ADDR,start_addy); 
			...
			//write(fd_sensor,&data,1);
			for(int k = 0; k < 16; k++){
				buf[k+1] = dmpMem[i][j][k];
				//	write(fd_sensor,&data,1);
				//Wire.send((uint8_t) byteToSend);
			}
			//Wire.endTransmission();
			write(fd_sensor,buf,17);
		}
...
uint8_t buf[40];
	...
	for(uint8_t i = 0; i < 22; i++){
		...
		regWrite(MEM_START_ADDR,dmp_updates[i][1]);
		//Wire.endTransmission();
		//Wire.beginTransmission(MPU_ADDR);
		//Wire.send(MEM_R_W);
		buf[0] = MEM_R_W;
		for(uint8_t j = 0; j < dmp_updates[i][2]; j++){
			buf[j+1] = dmp_updates[i][j+3];
			//Wire.send(dmp_updates[i][j+3]);
		}
		write(fd_sensor,buf,dmp_updates[i][2]+1);
		//Wire.endTransmission();
	}
...
/*
   void regWrite(uint8_t addy, uint8_t regUpdate){
//  Wire.beginTransmission(MPU_ADDR);
Wire.send(addy);
Wire.send(regUpdate);
//  Wire.endTransmission();
...
void getPacket(){
	...
	if(longPacket){
		...
		for(uint8_t i = 32; i < fifoCountL; i++){
			received_packet[i] = Wire.receive();
		}
		longPacket = false;
	}
		...
		for(uint8_t i = 0; i < fifoCountL; i++){
			received_packet[i] = Wire.receive();
		}
	}
}
...
uint8_t read_interrupt(){
	uint8_t int_status = regRead(0x3A);
	return int_status;
}
...
uint8_t fifoReady(){
	...
	//  Wire.endTransmission();
	uint8_t fifoCountH = Wire.receive();
...
void sendPacket(){
	for(uint8_t i = 0; i < fifoCountL-1; i++){
